...
def _avg_pool_b01(ArrayData imgs, int n_imgs, img_shape, win_shape, padding,
    strides, ArrayData out):
    cdef int img_h = img_shape[0]
    cdef int img_w = img_shape[1]
    cdef int win_h = win_shape[0]
    cdef int win_w = win_shape[1]
    ...
    cdef int stride_x = strides[1]
    if is_float(imgs):
        nnet.avg_pool_b01(float_ptr(imgs), n_imgs, img_h, img_w,
            win_h, win_w, pad_y, pad_x, stride_y, stride_x,
            float_ptr(out))
    else:
        raise ValueError('type %s not implemented' % str(imgs.dtype))
...
def _avg_pool_b01_bprop(ArrayData out_d, int n_imgs, img_shape, win_shape,
    padding, strides, ArrayData imgs_d):
    cdef int img_h = img_shape[0]
    cdef int img_w = img_shape[1]
    cdef int win_h = win_shape[0]
    ...
    cdef int stride_x = strides[1]
    if is_float(out_d):
        nnet.avg_pool_b01_bprob(float_ptr(out_d), n_imgs, img_h, img_w, win_h,
            win_w, pad_y, pad_x, stride_y, stride_x, float_ptr(imgs_d))
    else:
        raise ValueError('type %s not implemented' % str(out_d.dtype))
